...
from host import buzzer_server
...
def game(request, game_num):
    return render(request, 'host/play.html', {'game_num': game_num})


def double(request, game_num):
    return render(request, 'host/play.html', {'game_num': game_num, 'double': True})
...
def final(request, game_num):
    ...
    game.played = True
    game.save()
    return render(request, 'host/final.html',
